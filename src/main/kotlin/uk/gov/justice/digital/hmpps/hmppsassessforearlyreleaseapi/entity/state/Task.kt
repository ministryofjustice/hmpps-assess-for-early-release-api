...
enum class Task {
  ...
  ASSESS_ELIGIBILITY {
    ...
    override fun taskStatus(assessment: Assessment) = when (assessment.eligibilityChecksStatus) {
  ...
  ENTER_CURFEW_ADDRESS {
    ...
    override fun taskStatus(assessment: Assessment) = when (assessment.status) {
  ...
  SEND_CHECKS_TO_PRISON {
    ...
    override fun taskStatus(assessment: Assessment) = when {
      !assessment.status.isOneOf(AWAITING_ADDRESS_AND_RISK_CHECKS, ADDRESS_AND_RISK_CHECKS_IN_PROGRESS) -> UNAVAILABLE
