insert into staff (id, staff_identifier, staff_code, forename, surname, username, email, kind)
values (1, 49, 'STAFF1', 'a', 'com', 'a-com', 'a-com@justice.gov.uk', 'COMMUNITY_OFFENDER_MANAGER');

insert into staff (id, staff_identifier, staff_code, forename, surname, username, email, kind)
values (2, 792, 'STAFF2', 'another', 'com', 'another-com', 'another-com@justice.gov.uk', 'COMMUNITY_OFFENDER_MANAGER');
...
values ((select id from offender where booking_id = 10), 'ADDRESS_AND_RISK_CHECKS_IN_PROGRESS', '1.0', 2),
       ((select id from offender where booking_id = 20), 'ADDRESS_AND_RISK_CHECKS_IN_PROGRESS', '1.0', 1),
       ((select id from offender where booking_id = 30), 'AWAITING_ADDRESS_AND_RISK_CHECKS', '1.0', 1),
       ((select id from offender where booking_id = 40), 'AWAITING_ADDRESS_AND_RISK_CHECKS', '1.0', 2),
       ((select id from offender where booking_id = 50), 'NOT_STARTED', '1.0', 1),
       ((select id from offender where booking_id = 60), 'AWAITING_DECISION', '1.0', 2),
