...
class OpenApiDocsTest : SqsIntegrationTestBase() {
  ...
  fun `the open api json path security requirements are valid`() {
    ...
    result.openAPI.paths
      ...
      .forEach { pathItem ->
        pathItem.value.get?.let {
          assertThat(it.security?.flatMap { security -> security.keys }).describedAs { pathItem.toString() }
            .isSubsetOf(securityRequirements)
        }
        pathItem.value.post?.let {
          assertThat(it.security?.flatMap { security -> security.keys }).describedAs { pathItem.toString() }
            .isSubsetOf(securityRequirements)
        }
      }
  ...
  @ParameterizedTest
  @CsvSource(value = ["assess-for-early-release-admin-role, ROLE_ASSESS_FOR_EARLY_RELEASE_ADMIN"])
